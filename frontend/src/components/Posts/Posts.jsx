...
export default function Posts() {
  ...
  return (
       <div className="min-h-screen bg-gradient-to-r from-gray-300 via-purple-300 to-blue-300 p-4 sm:p-6 rounded-lg">
  <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-purple-700 text-center sm:text-left">
    All Posts
  </h1>
  ...
  {posts.map((post) => (
    <div
      key={post._id}
      className="bg-purple-100 shadow-lg rounded-2xl p-4 sm:p-6 mb-6 border border-gray-200 transition-transform hover:scale-[1.02] w-full max-w-3xl mx-auto"
    >
      {/* User info */}
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="truncate">
          <Link
            ...
            className="font-semibold text-purple-800 hover:underline block"
          >
          ...
          >
            <p className="text-sm text-gray-500 break-all">{post.user?.email}</p>
          </Link>
        </div>
      </div>

      {/* Post text */}
      <p className="text-gray-800 mb-3 break-words">{post.text}</p>
      ...
      {post.image && (
        <div className="flex justify-center mb-4">
          <img
            src={post.image}
            alt="Post"
            className="max-h-72 w-full sm:w-auto rounded-xl shadow-md object-cover"
          />
        </div>
      )}
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-1 border border-gray-400 rounded-full px-4 py-2 focus:ring focus:ring-purple-300 focus:outline-none text-black w-full"
            />
            <button
              onClick={() => addComments(post._id)}
              className="px-5 py-2 bg-purple-600 text-white rounded-full hover:bg-purple-700 transition active:scale-95 w-full sm:w-auto"
            >
              Submit
            </button>
          </div>
            ...
            {post.comments?.map((c) => (
              ...
              >
                <p className="text-sm text-black break-words">
                  <strong className="text-purple-700">{c.user?.username}</strong>
                  : {c.text}
                </p>
                {c.user?._id === user?.id && (
                  <button
